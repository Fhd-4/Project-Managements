...
// Programs have no owner field of their own — "Program Owner" is the owning
// Portfolio's ownerName (per your instruction). This wraps PortfolioService
// with an in-memory cache keyed by portfolioId so viewing multiple programs
// under the same portfolio doesn't re-fetch it every time.
@Injectable({
  providedIn: 'root'
})
