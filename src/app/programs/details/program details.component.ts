...
import { PortfolioLookupService } from '../../portfolios/portfolio.service';
import { ProjectService, ProjectSummary } from '../../project/project.service';
...
export class ProgramDetailsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.programService.getProgramDetails(id).subscribe({
      next: (program) => {
        ...
        // Portfolio Owner — Programs have no owner of their own; the owning
        // Portfolio's ownerName is used instead (cached to avoid refetching
        // the same portfolio across multiple program views).
  ...
  // API doesn't return a settable progressPercentage on create; fall back to
  // the confirmed status-derived value (70/100/0/0) per your instruction to
  // use frontend logic for this.
  get progress(): number { return this.program?.progressPercentage ?? this.statusMeta.progress; }

  projectStatusMeta(status: number) { return getStatusMeta(status); }
